...
import {
  ...
  findIdAndUpdate,
  getAllEspecialPrice,
} from "../services/priceSpecial.service.js"; // Importa las funciones del servicio para manejar precios especiales
...
export const priceSpecial = async (req: Request, res: Response) => {
  // Extrae los datos del cuerpo de la solicitud
  const { userId, productId, specialPrice, startDate, endDate, status } =
    req.body;
...
export const allSpecialPrice = async (req: Request, res: Response) => {
  try {
...
export const updateSpecialPrice = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!resp) res.status(404).json({ message: "No encontrado" });
