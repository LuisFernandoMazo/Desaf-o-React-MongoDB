...
export const findByIdentificacionAnduserId = async (identificacion: string) => {
  try {
    ...
    if (!resp?._id) return "usuario no existe";
    ...
    const updateProducts = listProduct?.map((product) => {
      ...
      const newPrice = userExist?.find(
        (user) => user.productId === product._id.toString()
      );
      ...
      return {
        ...product.toObject(),
        price:
          newPrice && newPrice.status ? newPrice.specialPrice : product.price,
      };
    });
